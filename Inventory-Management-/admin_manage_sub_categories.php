...
{
	$id = $_GET["did"];
	deleteFromDB("delete from subcategory where subcategory_id = $id");
	header("Location: admin_manage_sub_categories.php");
}
...
<head>
	...
	<!-- Customized css file -->
	<link rel="stylesheet" type="text/css" href="styles/admin_manage_sub_categories.css" />
		...
		<div class="content">
			<div class="content_area">
				...
				<table id="myTable">
					...
					<?php 
						$result = getJSONFromDB("select * from subcategory order by SUBCATEGORY_ID asc");
						$result = json_decode($result, true);
						for($i=0;$i<sizeof($result);$i++){
							$sid = $result[$i]["SUBCATEGORY_ID"];
							...
							echo "<tr>
								...
								<td>$cid</td>
								<td>$name</td>
								<td><a href='admin_manage_sub_categories.php?did=$sid'>Delete</a></td>
							 </tr>";
						}
					?>
