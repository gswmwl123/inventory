...
<head>
	...
	<!-- Customized css file -->
	<link rel="stylesheet" type="text/css" href="styles/admin_material_list.css" />
			...
			<div class="content_area">
				...
				<table id="myTable">
					<tr class="header">
						...
						<th style="width:9%;">VENDOR_ID</th>
						<th style="width:9%;">MATERIAL_NAME</th>
						<th style="width:5%;">Stock</th>
						<th style="width:7%;">UNIT PRICE</th>
						<th style="width:7%;">PURCHASE DATE</th>
						<th style="width:15%;">Action</th>
					</tr>
	
					<?php 
						$result = getJSONFromDB("select * from raw_materials order by MATERIAL_ID asc");
						$result = json_decode($result, true);
						for($i=0;$i<sizeof($result);$i++){
							$id = $result[$i]["MATERIAL_ID"];
							$mid = $result[$i]["MANAGER_ID"];
							$vid = $result[$i]["VENDOR_ID"];
							$name = $result[$i]["MATERIAL_NAME"];
							$stock =$result[$i]["STOCK"];
							$price = $result[$i]["UNIT_PRICE"];
							$date = $result[$i]["PURCHASE_DATE"];
							echo "<tr>
								<td>$id</td>
								<td>$mid</td>
								<td>$vid</td>
								...
								<td>$price</td>
								<td>$date</td>
								<td><a href='admin_material_list.php?did=$id'>Delete</a></td>
							 </tr>";
						}
					?>
